...
def molfrac_to_wtpercent(molfrac):
    ...
    for key, value in molfrac.items():
        MPO_dict[key] = value * core.oxideMass[key]
...
def calc_gammas(temp, press, species='all'):
        ...
        -------
        float or dict
            Fugacity coefficient for passed species. If single species is passed, float. If "all" is
            passed, dictionary with keys 'CO', 'CO2', 'H2', 'H2O', 'H2S', 'O2', 'S2', 'SO2'.
        """
...
class MagmaticFluid(object):
    ...
    def __init__(self, fluid_comp, press, temp, fO2_buffer, fO2_delta, input_type='wtpercent'):
        ...
        self.logfO2 = calc_logfO2_from_buffer(press=press, temp=temp, buffer=fO2_buffer,
                                              delta=fO2_delta)
    ...
    def fugacities(self, gammas='calculate', K_vals='calculate'):
        ...
        def equations(p):
            ...
            return  [
                        ((fH2/(gammas['H2'] * press)) +
                            ((Rational(2.0) * K_vals['H2O'] * fH2 * sqrt(fO2)) /
                             (Rational(3.0) * gammas['H2O'] * press))   +
                            ((Rational(2.0) * K_vals['H2S'] * fH2 * sqrt(abs(fS2))) /
                             (Rational(3.0) * gammas['H2S'] * press)) -
                            XHtot), 
                        ((fS2/(gammas['S2'] * press)) +
                            ((K_vals['H2S'] * fH2 * sqrt(abs(fS2))) /
                             (Rational(3.0) * gammas['H2S'] * press)) + 
                            ((K_vals['SO2'] * fO2 * sqrt(abs(fS2))) /
                             (Rational(3.0) * gammas['SO2'] * press)) -
                            XStot)
                    ]
        ...
        if XCtot  == 0:
            ...
            equation = (((K_vals['CO2'] * fCO * sqrt(fO2)) /
                        (Rational(3.0) * gammas['CO2'] * press)) +
    ...
    def speciate(self, return_as='wtpercent', gammas='calculate', K_vals='calculate',
                 fugacities='calculate'):
...
class Model(object):
    ...
    def __init__(self, press, temp, fO2_buffer, fO2_delta, H2O_param, H2O_param_type, SC_param,
                 SC_param_type):
    ...
    def eq_fluid(self, return_as='molfrac'):
        ...
        -------
        dict
            Dictionary with composition of equilibrium fluid as: {'CO': value, 'CO2': value,
            'H2': value, 'H2O': value, 'H2S': value, 'O2': value, 'S2': value, 'SO2': value}
        """
        ...
        equation = ((fS2 / gS2) + (KSO2 * sqrt(fS2) * fO2 / gSO2) + 
                    (KH2S * fH2 * sqrt(fS2) / gH2S) - PStot)
    ...
    def respeciate(self, fluid_comp, newP, input_type='wtpercent'):
        ...
        logfO2_res = calc_logfO2_from_buffer(press=newP, temp=self.temp, buffer=self.fO2_buffer,
                                             delta=self.fO2_delta)
