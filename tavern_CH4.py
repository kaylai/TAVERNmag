...
oxideMass = {'SiO2':28.085+32.0,'MgO':24.305+16.0,'FeO':55.845+16.0,'CaO':40.078+16.0,'Al2O3':2.0*26.982+16.0*3.0,
			 ...
			 'Cl':35.45,'Fe2O3':55.845*2.0+16.0*3.0,'CH4': 16.04,'CO': 28.01,'CO2':44.01,'H2': 2.01588,'H2O':18.02,
			 'H2S':34.1, 'O':15.999, 'O2':15.999*2.0, 'S':32.065, 'Stot': 32.065, 'S2':32.065*2.0, 'SO2':32.065+32.0}

fluid_species_names = ['CH4', 'CO', 'CO2', 'H2', 'H2O', 'H2S', 'O2', 'S2', 'SO2']
...
def normalize(composition):
	...
	composition: dict
		...
		H2O, CH4, CO2, S, S2, CO, H2, H2S, SO2, O, O2
...
def wtpercent_to_molpercent(wtpercent):
	...
	wtpercent: dict
		Dictionary of a composition in wt percent with possible keys:
		SiO2, TiO2, Al2O3, FeO, Fe2O3, MgO, MnO, CaO, Na2O, K2O, P2O5, Cr2O3, NiO, F, Cl, 
		H2O, CH4, O2, S, S2, CO, H2, H2S, SO2, O, O2
...
def calc_Ks(temp, species='all', return_as='standard'):
		...
		species: str
			...
			Options are: CH4, CO2, H2O, H2S, SO2, or all.
		...
		if tempK <= 1500.0:
			# use 6-poly fit to log K data
			CH4_logK =		(-6.32167*np.power(10.,-17)	*	np.power(tempK,6.0)
						+	3.92831*np.power(10.,-13)	*	np.power(tempK,5.0)
						+	-1.00684*np.power(10.,-9)	*	np.power(tempK,4.0)
						+	1.37434*np.power(10.,-6)	*	np.power(tempK,3.0)
						+	-1.07547*np.power(10.,-3)	*	np.power(tempK,2.0)
						+	4.82213*np.power(10.,-1)	*	tempK
						+	-1.02408*np.power(10.,2))
		else:
			# use 6-poly fit to K data
			CH4_logK = 	np.log10(4.02935*np.power(10.,-12.0)	*	np.power(tempK,6.0)
						+	-1.90687*np.power(10.,-8.0)		*	np.power(tempK,5.0)
						+	3.61764*np.power(10.,-5) 		* 	np.power(tempK,4.0)
						+	-3.50123*np.power(10.,-2)		*	np.power(tempK,3.0)
						+	1.8121*np.power(10.,1)			*	np.power(tempK,2.0)
						+	-4.72716*np.power(10.,3)		*	tempK
						+	4.83107*np.power(10.,5))

		# CO + 1/2O2 = CO2
		"""From Wagman et al (1945). Text table was fit with a power curve to
		arrive at the following coefficients."""
		CO2_logK =	(8.10920*np.power(10.,4) * np.power(tempK,-1.30546*np.power(10.,00)))

 		# H2 + 1/2O2 = H2O
		"""From Robie and Hemingway (1995). Text table was fit with a power curve to
		arrive at the following coefficients."""
		H2O_logK = 	(4.71943*np.power(10.,4) * np.power(tempK,-1.23065*np.power(10.,00)))

		# H2 + 1/2S2 = H2S
		"""From Robie and Hemingway (1995). Text table was fit with a logarithmic curve to
		arrive at the following coefficients."""
		H2S_logK = 	(3.26559*np.power(10.,0)*np.log(tempK) + 2.45625*np.power(10.,1))

		# S2 + 1/2O2 = SO2
		"""From Robie and Hemingway (1995). Text table was fit with a power curve to
		arrive at the following coefficients."""
		SO2_logK =	(3.53001*np.power(10.,4) * np.power(tempK,-1.13166*np.power(10.,0)))

		if return_as == 'standard':
			if species == 'CH4':
				return 10.0**CH4_logK
			if species == 'CO2':
			...
			if species == 'all':
				return {'CH4': 10**CH4_logK, 'CO2': 10.0**CO2_logK, 'H2O': 10.0**H2O_logK, 'H2S': 10.0**H2S_logK, 'SO2': 10.0**SO2_logK}
		if return_as == 'log':
			if species == 'CH4':
				return CH4_logK
			if species == 'CO2':
			...
			if species == 'SO2':
				return SO2_logK
			if species == 'all':
				return {'CH4': CH4_logK, 'CO2': CO2_logK, 'H2O': H2O_logK, 'H2S': H2S_logK, 'SO2': SO2_logK}
...
def calc_gammas(temp, press, species='all'):
		...
		float or dict
			...
			If "all" is passed, dictionary with keys 'CH4', CO', 'CO2', 'H2', 'H2O', 'H2S', 'O2', 'S2', 'SO2'
		"""
...
class MagmaticFluid(object):
	...
	----------
	    ...
	    fO2_buffer: str?
	    	Name of the fO2 buffer relative to which you want to calculate an fO2.
	    	Only QFM is implemented currently. #TODO implement more buffers.
	    fO2_delta: float
	    	Deviation in log units from the fO2_buffer name supplied. For example,
	    	input fO2_buffer="QFM", fO2_delta=1 for an fO2 one log unit above the
	    	QFM buffer (QFM+1).
	    input_type: str
